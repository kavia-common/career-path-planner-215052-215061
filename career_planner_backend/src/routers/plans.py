...
async def list_plans(authorization: str = Header(...), user: AuthUser = Depends(supabase_user_from_jwt)):
...
async def create_plan(payload: PlanCreate, authorization: str = Header(...), user: AuthUser = Depends(supabase_user_from_jwt)):
    token = authorization.split(" ", 1)[1]
    client = SupabaseClient.user_mode(token)
...
async def list_goals(plan_id: int, authorization: str = Header(...), user: AuthUser = Depends(supabase_user_from_jwt)):
    token = authorization.split(" ", 1)[1]
    client = SupabaseClient.user_mode(token)
...
async def create_goal(plan_id: int, payload: GoalCreate, authorization: str = Header(...), user: AuthUser = Depends(supabase_user_from_jwt)):
    token = authorization.split(" ", 1)[1]
    client = SupabaseClient.user_mode(token)
